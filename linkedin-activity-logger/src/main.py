...
from src.importers.linkedin_api_importer import LinkedInAPIImporter
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compliant LinkedIn Activity Logger")
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("init-db", help="Initialize SQLite database")

    import_archive_parser = subparsers.add_parser(
        "import-archive", help="Import LinkedIn data archive files"
    )
    import_archive_parser.add_argument("--path", required=True, help="Path to LinkedIn archive directory")

    import_api_parser = subparsers.add_parser(
        "import-api", help="Import from official LinkedIn API where permissions allow"
    )
